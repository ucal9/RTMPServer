...
void CSimpleBuffer::Extend(uint32_t len) {
    ...
    alloc_size_ += alloc_size_ >> 2; // 增加1/4的分配大小
...
int CSimpleBuffer::Skip(uint32_t len) { 
...
void CByteStream::_ReadByte(void *buf, uint32_t len) {
    if (pos_ + len > len_) {
        throw CPduException(ERROR_CODE_PARSE_FAILED, "解析数据包失败!");
    }
...
/*
 * 警告!!!
 * 此函数返回一个静态字符指针，调用者必须立即将字符串复制到其他位置
 */
